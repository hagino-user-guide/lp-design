...
/* v64: seamless overview-to-four-reasons scroll transition */
        ...
        <div class="comparison-transition">
          ...
          <div class="ashita-focus-card" aria-hidden="true">
            ...
            <div class="focus-about">
              ...
              <p class="about-next">そのままスクロールして、<br>選ばれる4つの理由へ</p>
            </div>
          </div>
        </div>
        </div>
...
const meritHeading=document.querySelector('.merit-heading-wrap');
...
const updateScroll=()=>{
  ...
  if(comparisonTransition && comparisonBox && focusCard){
    ...
    if(meritHeading){
      const meritIn=clamp((cp-.78)/.18);
      meritHeading.style.opacity=String(meritIn);
      meritHeading.style.transform=`translateY(${52*(1-meritIn)}px) scale(${.94+.06*meritIn})`;
      meritHeading.style.filter=`blur(${5*(1-meritIn)}px)`;
    }
  }
};
