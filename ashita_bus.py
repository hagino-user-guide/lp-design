...
.hero-stage{position:sticky;position:-webkit-sticky;top:0;height:100svh;min-height:100svh;overflow:hidden;background:#061326;z-index:1}
.hero-photo{position:absolute;inset:-3%;background-image:linear-gradient(rgba(3,13,30,.10),rgba(3,13,30,.28)),url('__INTERIOR__');background-size:cover;background-position:center 45%;transform:scale(1.02);transform-origin:50% 30%;will-change:transform,filter,opacity}
...
.hero-card h1{width:100%;margin:0;font-family:"Hiragino Kaku Gothic ProN","Yu Gothic","YuGothic",Meiryo,sans-serif;font-size:clamp(33px,7.5vw,41px);line-height:1.2;letter-spacing:-.04em;font-weight:800;font-feature-settings:"palt" 1;word-break:keep-all;text-align:center;color:#102f63}
.hero-card h1 span{display:block;width:100%;white-space:nowrap}
.hero-card h1 .thanks{margin-top:4px}
.hero-card p{margin:18px 0 0;font-size:15px;line-height:1.85;font-weight:650;letter-spacing:.01em;color:#223b5d}
.hero-hook{position:absolute;z-index:5;left:50%;bottom:4.6svh;width:min(84%,330px);transform:translateX(-50%) translateY(14px);padding:18px 18px 12px;border:1px solid rgba(255,255,255,.24);border-radius:22px;background:linear-gradient(180deg,rgba(4,20,45,.20),rgba(4,20,45,.68));text-align:center;color:#fff;opacity:.42;will-change:opacity,transform;filter:drop-shadow(0 8px 18px rgba(0,0,0,.26));backdrop-filter:blur(5px)}
.hero-hook .hook-label{display:block;margin-bottom:7px;color:#a9dfff;font-size:11px;line-height:1;font-weight:800;letter-spacing:.12em}.hero-hook strong{display:block;font-size:18px;line-height:1.55;letter-spacing:.01em;font-weight:900;text-shadow:0 3px 16px rgba(0,0,0,.78)}
.hero-hook .chevron{display:block;margin-top:5px;font-size:42px;line-height:.8;font-weight:900;animation:chevron 1.7s ease-in-out infinite}
@keyframes chevron{0%,100%{transform:translateY(0);opacity:.55}50%{transform:translateY(8px);opacity:1}}

/* v59: tightened CTA city-and-bus composition */
...
.cell{min-height:128px;padding:12px 6px;display:grid;place-items:center;text-align:center;border-right:1px solid #e1e7ee;border-bottom:1px solid #e1e7ee;font-size:11px;line-height:1.5;font-weight:750;color:#334154}
...
.cell strong{display:block;margin-bottom:6px;color:var(--pink);font-size:18px;line-height:1.16;text-shadow:0 2px 0 #fff}
...
.comparison-transition{position:relative;height:172svh;margin-top:22px}
...
.focus-about{position:absolute;z-index:3;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:34px 25px;text-align:center;opacity:0;transform:translateY(14px) scale(.96);will-change:opacity,transform;background:linear-gradient(180deg,#f8fcff 0%,#eef8ff 100%)}
.focus-about .about-logo{width:172px;margin:0 auto 20px}
...
.focus-about .about-main{margin:20px 0 0;color:#163a6f;font-size:17px;line-height:1.9;font-weight:850}
.focus-about .about-next{position:relative;margin:18px 0 0;padding:14px 18px 28px;border:0;border-radius:19px;background:rgba(255,255,255,.72);color:var(--blue);font-size:15px;line-height:1.65;font-weight:900;box-shadow:0 9px 22px rgba(22,95,227,.10)}
...
.merit-section .merit-heading-wrap{animation:meritBurst .48s cubic-bezier(.18,.85,.25,1) both}
...
.merit-card-icon svg{width:46px;height:46px}
        ...
        <div class="comparison-transition">
          ...
          <div class="ashita-focus-card" aria-hidden="true">
            ...
            <div class="focus-about">
              ...
              <p class="about-next">続いて、あしたのバスが選ばれる<br>4つの理由をご紹介！！</p>
            </div>
          </div>
        </div>
        </div>
...
const updateScroll=()=>{
  ...
  if(comparisonTransition && comparisonBox && focusCard){
    ...
    const focusExit=clamp((cp-.68)/.10);
    ...
    const targetHeight=Math.min(innerHeight*.72,570);
    // Center the expanded card so the screen never becomes a large empty white area.
    const targetTop=Math.max(10,(comparisonBox.clientHeight-targetHeight)/2);
    ...
    focusAbout.style.transform=`translateY(${8*(1-aboutIn)-5*focusExit}px) scale(${.985+.015*aboutIn-.005*focusExit})`;
    focusCard.classList.toggle('reading-hold',cp>=.54 && cp<.94);
  }
};
addEventListener('scroll',updateScroll,{passive:true});
updateScroll();

const meritCards=document.querySelectorAll('.reveal-card');
const meritCardObserver=new IntersectionObserver((entries)=>{
  entries.forEach((entry)=>{
    if(entry.isIntersecting){
      entry.target.classList.add('show');
      meritCardObserver.unobserve(entry.target);
    }
  });
},{threshold:.16,rootMargin:'0px 0px -6% 0px'});
meritCards.forEach((card,index)=>{
  card.style.transitionDelay=`${Math.min(index*.05,.15)}s`;
  meritCardObserver.observe(card);
});
</script>
</body>
</html>'''
...
def render() -> None:
    try:
        ...
        st.stop()
    st.components.v1.html(build_html(assets), height=920, scrolling=True)
