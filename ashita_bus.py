...
.hero-hook{position:absolute;z-index:5;left:50%;bottom:2.4dvh;width:min(84%,320px);transform:translateX(-50%) translateY(14px);padding:13px 16px 9px;border:1px solid rgba(255,255,255,.24);border-radius:22px;background:linear-gradient(180deg,rgba(4,20,45,.20),rgba(4,20,45,.68));text-align:center;color:#fff;opacity:.42;will-change:opacity,transform;filter:drop-shadow(0 8px 18px rgba(0,0,0,.26));backdrop-filter:blur(5px)}
...
/* v62: responsive iPhone layouts across hero, comparison, merits, and CTA */
...
const updateScroll=()=>{
  ...
  if(comparisonTransition && comparisonBox && focusCard){
    ...
    const targetHeight=Math.min(innerHeight*.84,610);
    // Center the expanded card so the screen never becomes a large empty white area.
    const targetTop=Math.max(4,(comparisonBox.clientHeight-targetHeight)/2);
